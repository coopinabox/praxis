...
describe("TaskListView", function() {
  ...
  before(function() {
    ...
    taskListView = new TaskListView({ model: taskList });
  ...
  describe('clicking the plus button twice', function() {
    ...
    before(function() {
      ...
      taskListView.getDOMNode().querySelector('button.add').click();
      taskListView.getDOMNode().querySelector('button.add').click();
    });

    it("should create two empty tasks", function() {
      expect(taskList.add.calledTwice).to.be.true;
      expect(taskListView.getDOMNode().querySelectorAll('form.task-form').length).to.equal(2);
