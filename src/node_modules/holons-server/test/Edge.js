  ...
  describe("relations", function () {
    ...
    describe("#successors", function () {
      ...
      it("the successors of a12 should be [a23]", function () {
        return self.a12.successors().fetch().then(function (edges) {
          // TODO to.have.deep.members after https://github.com/chaijs/chai/pull/228
          expect(edges.toJSON()).to.deep.equal([self.a23.toJSON()]);
        });
      });

      it("the successors of a23 should be [a34]", function () {
        return self.a23.successors().fetch().then(function (edges) {
    ...
    describe("#predecessors", function () {
      ...
      it("the predecessors of a23 should be [a12]", function () {
        return self.a23.predecessors().fetch().then(function (edges) {
          expect(edges.toJSON()).to.deep.equal([self.a12.toJSON()]);
        });
      });

      it("the predecessors of a34 should be [a23]", function () {
        return self.a34.predecessors().fetch().then(function (edges) {
          expect(edges.toJSON()).to.deep.equal([self.a23.toJSON()]);
        });
      });
    });
